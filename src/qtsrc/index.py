# code:utf-8
from PyQt5.QtWidgets import *
from PyQt5.QtGui import *
from PyQt5.QtCore import *
import sys
import time
from src.getdata.get_data import GetStockData


class MyTable(QTableWidget):
    def __init__(self, parent=None):
        super(MyTable, self).__init__(parent)
        self.setWindowTitle("实时信息")  # 设置表格名称
        self.setWindowIcon(QIcon("ok.png"))  # 设置图标（图片要存在）
        self.resize(500, 180)  # 设置表格尺寸（整体大小）
        self.setColumnCount(3)  # 设置列数
        self.setRowCount(2)  # 设置行数
        # self.setColumnWidth(0, 200)  # 设置列宽(第几列， 宽度)
        # self.setRowHeight(0, 100)  # 设置行高(第几行， 行高)

        column_name = [
            '名称/代码',
            '价格',
            '涨幅'
        ]
        self.setHorizontalHeaderLabels(column_name)  # 设置列名称
        row_name = [
            '01',
            '02'
        ]
        self.setVerticalHeaderLabels(row_name)  # 设置行名称

    def update_item_data(self, data):
        """更新内容"""
        name = data['股票名称']
        price = data['当前价格']
        zf = data['涨幅']
        # 设置第一行
        self.setItem(0, 0, QTableWidgetItem(name))  # 设置表格内容(行， 列) 文字
        self.setItem(0, 1, QTableWidgetItem(price))
        self.setItem(0, 2, QTableWidgetItem(zf))


class UpdateData(QThread):
    """更新数据类"""
    # update_date = pyqtSignal(str)  # pyqt5 支持python3的str，没有Qstring
    update_date = pyqtSignal(dict)

    def __init__(self, data_service: GetStockData, code: str):
        super().__init__()
        self.data_service = data_service
        self.code = code

    def run(self):
        cnt = 0
        while True:
            cnt += 1
            data = self.data_service.get_tick_data(self.code)
            self.update_date.emit(data)  # 发射信号
            time.sleep(1)


if __name__ == '__main__':
    # 实例化表格
    app = QApplication(sys.argv)
    myTable = MyTable()

    # 启动更新线程
    stock_url = 'http://hq.sinajs.cn/list='
    update_data_thread = UpdateData(GetStockData(stock_url), code='600519')
    update_data_thread.update_date.connect(myTable.update_item_data)  # 链接信号
    update_data_thread.start()

    # 显示在屏幕中央
    desktop = QApplication.desktop()  # 获取坐标
    x = (desktop.width() - myTable.width()) // 2
    y = (desktop.height() - myTable.height()) // 2
    myTable.move(x, y)  # 移动

    # 显示表格
    myTable.show()
    app.exit(app.exec_())
